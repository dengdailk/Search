...
public class SearchBox extends RelativeLayout {
    ...
    private final MaterialMenuView materialMenu;
    ...
    public SearchBox(final Context context, AttributeSet attrs, int defStyle) {
        ...
        overflow.setOnClickListener(v -> popupMenu.show());
...
//                    mic.setImageDrawable(ContextCompat.getDrawable(context,R.drawable.ic_clear));
        ...
        mSearchFilter = (searchResult, searchTerm) -> searchResult.title.toLowerCase()
    ...
    private static boolean isIntentAvailable(Context context, Intent intent) {
        ...
        if (mgr != null) {
            List<ResolveInfo> list = Collections.unmodifiableList(mgr.queryIntentActivities(intent, PackageManager.MATCH_DEFAULT_ONLY));
            return list.size() > 0;
        }
        return false;
    }
    ...
    public void hideCircularly(int x, int y, Activity activity) {
        final FrameLayout layout = activity.getWindow().getDecorView()
                .findViewById(android.R.id.content);
        RelativeLayout root = findViewById(R.id.search_root);
